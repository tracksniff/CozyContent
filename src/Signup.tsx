...
const Signup: React.FC = () => {
    ...
    <div className="flex justify-between pt-4">
      ...
      {isSubmit ? (
        <button type="submit" disabled={isLoading}
          ...
          {isLoading ? <><Loader2 size={18} className="animate-spin" /> Submitting…</> : 'Review Pricing'}
        </button>
      ) : (
        <button type="button" onClick={onNext} disabled={canNext === false}
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-surface transition-colors duration-300 p-6">
      ...
      <Link to="/" className="absolute top-8 left-8 flex items-center gap-2 text-on-surface-variant hover:text-primary transition-colors font-bold text-sm">
          ...
          <div className="text-center mb-10">
            ...
            <div className="flex items-center justify-center gap-2 mt-6">
              {Array.from({ length: TOTAL_STEPS }, (_, i) => i + 1).map(s => (
                <div key={s} className={`h-1.5 rounded-full transition-all duration-300 ${s <= step ? 'w-8 bg-primary' : 'w-4 bg-outline-variant'}`} />
              ))}
            </div>
            <p className="mt-2 text-[10px] text-on-surface-variant font-bold uppercase tracking-widest">Step {step} of {TOTAL_STEPS}</p>
          </div>
            ...
            {step === 1 && (
              <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div>
                    <label className="block text-xs font-black uppercase tracking-widest text-on-surface-variant mb-2 ml-1">Phone Number *</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input name="email" type="email" required placeholder="you@example.com" className={inputCls} value={formData.email} onChange={handleInput} />
                  </div>
                  <div>
                    <label className="block text-xs font-black uppercase tracking-widest text-on-surface-variant mb-2 ml-1">
                      Current Website <span className="normal-case font-normal">(Optional)</span>
                    </label>
                    <input name="website_url" placeholder="https://example.com" className={inputCls} value={formData.website_url} onChange={handleInput} />
                  </div>
                </div>
            ...
            {step === 2 && (
              <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
                <div>
                  ...
                  <textarea name="services_list" required rows={3}
                    placeholder="e.g. Emergency Callouts, Boiler Repairs, Drain Unblocking, Bathroom Fitting…"
                    className={inputCls} value={formData.services_list} onChange={handleInput} />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div>
                    <label className="block text-xs font-black uppercase tracking-widest text-on-surface-variant mb-2 ml-1">
                      Years of Experience <span className="normal-case font-normal">(Optional)</span>
                    </label>
                    <input name="years_experience" placeholder="e.g. 15+" className={inputCls} value={formData.years_experience} onChange={handleInput} />
                  </div>
                </div>
                  ...
                  <div className="rounded-2xl overflow-hidden border border-outline-variant shadow-md select-none" aria-hidden="true">
                    ...
                    <div className="px-3 py-2 bg-surface-container-low border-b border-outline-variant flex items-center gap-2">
                      ...
                      <div className="flex-1 flex justify-center">
                        <div className="h-4 w-40 rounded-full bg-outline-variant/40 flex items-center justify-center">
                          <span className="text-[7px] text-on-surface-variant font-medium">your-website.com</span>
                        </div>
                      </div>
                    ...
                    <div className="px-4 py-2.5 flex items-center justify-between" style={{ backgroundColor: brandColors.secondary }}>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="hidden sm:flex gap-2">
                          {[1, 2, 3].map(i => <div key={i} className="h-1.5 w-7 rounded-full bg-white/30" />)}
                        </div>
                        <div className="h-6 w-16 rounded-full text-[8px] flex items-center justify-center font-black"
                          style={{ backgroundColor: brandColors.accent, color: isLight(brandColors.accent) ? '#111' : 'white' }}>
                          Call Now
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="px-5 py-8 flex flex-col gap-2.5"
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        <div className="h-8 w-28 rounded-xl text-[9px] flex items-center justify-center font-black shadow-md"
                          style={{ backgroundColor: brandColors.accent, color: isLight(brandColors.accent) ? '#111' : 'white' }}>
                          Get a Free Quote
                        </div>
                      ...
                      <div className="mt-3 flex gap-2">
                        {[{ v: '500+', l: 'Jobs Done' }, { v: '4.9★', l: 'Rating' }, { v: '24/7', l: 'Available' }].map(s => (
                          <div key={s.l} className="flex-1 bg-white/15 backdrop-blur-sm rounded-xl p-2 flex flex-col items-center gap-0.5">
                      ...
                      {[1, 2, 3].map(i => (
                        <div key={i} className="rounded-xl p-2.5 flex flex-col gap-1.5 border border-gray-100 shadow-sm">
                          <div className="w-6 h-6 rounded-lg flex items-center justify-center"
                            ...
                            <div className="w-2.5 h-2.5 rounded" style={{ backgroundColor: brandColors.primary }} />
                          </div>
            ...
            {step === 4 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                  ...
                  <div className="flex items-start gap-4">
                    {logoPreview ? (
                      ...
                      <label className="w-20 h-20 flex-shrink-0 rounded-2xl border-2 border-dashed border-outline-variant flex flex-col items-center justify-center gap-1 cursor-pointer hover:border-primary hover:bg-primary/5 transition-all">
                        ...
                        <span className="text-[9px] font-black uppercase tracking-widest text-on-surface-variant">Logo</span>
                ...
                <div>
                  ...
                  <p className="text-[10px] text-on-surface-variant font-medium">Team photos, premises, work examples — these will appear throughout your site.</p>
                </div>
